...
//
// The protocol was reverse-derived from the reference client shipped in WinlatorXR/HaloCEWXR
// (HaloCEVR/VR/WinXrApi.cpp and WinXrApiUDP.cpp) and first used by the Far Cry VR mod. XrAPI is
// explicitly labelled experimental (v0.5) by its authors, so this may need updating against future
// WinlatorXR versions.
namespace WinlatorXR
{
	struct HandState
	{
		...
		// OpenXR *grip* pose orientation (protocol >= 0.5 only, see hasGripOrientation). This is what
		// the mod's OpenXR path binds (/input/grip/pose), so it is what the hand/weapon code is tuned for.
	...
	struct InputState
	{
		bool valid = false;
		int frameId = -1;
		// GetTickCount() at the time the packet was received (for finite-difference velocities; wraps every 49 days)
		unsigned long receiveTimeMs = 0;
		...
		// WinlatorXR sends raw OpenXR data (x = right, y = up, -z = forward, metres): the HMD quaternion
		// is the left-eye view orientation, the HMD position is the midpoint between both eye views,
		// and ipd is the distance between the two eye views.
		float ipd = 0.064f;
	...
	// filesystem and is not normally present on a native Windows install. The environment variable
	// CRYSISVR_WINLATORXR overrides the heuristic (0 = force off, 1 = force on) for testing.
	...
	// in alternate-eye mode B > 0 marks the frame as the right eye.
