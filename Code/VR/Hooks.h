...
namespace hooks {
	...
	template <typename T, typename R, typename... Args>
	void InstallVirtualFunctionHook(const char* name, T* instance, R (T::*func)(Args...), R (*detour)(T*, Args...))
	{
		int vtableIndex = getVTableIndex(func);
		CryLogAlways("Found vtable index %i for hook %s", vtableIndex, name);
		InstallVirtualFunctionHook(name, (void*)instance, vtableIndex, (void*)detour);
	}

	template <typename T, typename R, typename... Args>
	void InstallVirtualFunctionHook(const char* name, T* instance, R (T::*func)(Args...) const, R (*detour)(T*, Args...))
	{
