...
// This translation unit does not get <windows.h> from StdAfx.h, so pull in Winsock 2 (which includes
// <windows.h> itself) explicitly before anything else that might drag in the legacy <winsock.h>.
#include <winsock2.h>
#include <ws2tcpip.h>
#include <windows.h>
...
namespace WinlatorXR
{
	...
	static bool ParsePacket(char* text, InputState& out)
	{
		...
		out.receiveTimeMs = GetTickCount();
		out.valid = true;
		return true;
	}
	...
	static void WriteMarkerFiles()
	{
		...
		// (parsed in ParsePacket). WinlatorXR picks the implementation from this file once at startup.
	...
	bool IsLikelyPresent()
	{
		...
		if (GetEnvironmentVariableA("CRYSISVR_WINLATORXR", overrideValue, sizeof(overrideValue)) > 0)
		{
			return overrideValue[0] == '1';
		}
	...
	void Init()
	{
		...
		// flat "screen" mode (the intro videos and menu are 2D); the frame composer switches to
		// modeVr=1 per frame once it composes real stereo frames. fov 0/0 keeps the headset's native
		// FOV, which WinlatorXR then reports back to us in every packet.
