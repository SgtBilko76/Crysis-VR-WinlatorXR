...
void SCVars::InitVRCVars(IConsole* pConsole)
{
	...
	pConsole->Register("vr_winlatorxr_render_height", &vr_winlatorxr_render_height, 0, VF_RESTRICTEDMODE|VF_DUMPTODISK, "Per-eye render height when running under WinlatorXR (Quest/Pico); width follows the headset FOV aspect. 0 = the X screen height (full resolution, recommended)");
	pConsole->Register("vr_winlatorxr_fov_scale", &vr_winlatorxr_fov_scale, 1.0f, VF_RESTRICTEDMODE|VF_DUMPTODISK, "Under WinlatorXR, multiplier for the native FOV sent back to WinlatorXR as the display projection (1.0 = rendered FOV equals displayed FOV)");
	...
	pConsole->Register("vr_winlatorxr_2d_panel", &vr_winlatorxr_2d_panel, 1, VF_RESTRICTEDMODE|VF_DUMPTODISK, "Under WinlatorXR, show binoculars, weapon scopes and 2D cutscenes as a panel in a head-tracked VR view (like on PC) instead of on WinlatorXR's flat virtual screen");
	pConsole->Register("vr_winlatorxr_menu_panel", &vr_winlatorxr_menu_panel, 1, VF_RESTRICTEDMODE|VF_DUMPTODISK, "Under WinlatorXR, show menus and loading screens on a panel fixed in the room (like on PC) instead of on WinlatorXR's flat screen, which follows the head");
	pConsole->Register("vr_winlatorxr_menu_curve_radius", &vr_winlatorxr_menu_curve_radius, 5.0f, VF_RESTRICTEDMODE|VF_DUMPTODISK, "Under WinlatorXR, radius in metres of the curved screen menus and loading screens are shown on (0 = flat). The default equals the screen distance, so every part of the screen is equally far away");
